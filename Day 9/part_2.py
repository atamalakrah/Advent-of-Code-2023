...
#Calculates the difference between values in an array and returns the new list
#def find_diff_beginning(l):
#    diff_arr = []
#    counter = 0
#    for value in l:
#        while counter > len(l) :
#           diff_arr.append(l[] - l[0])
#            counter += 1
#    return diff_arr
...
for a in values:
    last_digit_total += a["value_array"][0][len(a["value_array"][0])-1]

print("Last digit total: ", last_digit_total)

#calculates the new first digit for part 2
for a in values:
    diff_arr_len = len(a['value_array'])
    while diff_arr_len > 1:
        curr_arr = a['value_array'][diff_arr_len-1]
        next_arr = a['value_array'][diff_arr_len-2]
        new_value = next_arr[0] - curr_arr[0]
        next_arr.insert(0, new_value)
        diff_arr_len -= 1

for a in values:
    first_digit_total += a["value_array"][0][0]
        

print("First digit total: ", first_digit_total)
