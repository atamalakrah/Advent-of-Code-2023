...
def valid_pattern_count (question_sections: list, pattern):
    ...
    for combo in combinations:
        current_combination = replace_question_sections(pattern, combo, question_sections)
        if is_valid_combination(list(current_combination), spring[1]):
...
for spring in springs:
    pattern = spring[0]
    question_sections = find_question_sections(pattern)        
